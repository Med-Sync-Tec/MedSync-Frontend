...
- Name: patients-list.spec.ts >> Lista de Pacientes >> UAT-PAT-11 | botón "Nuevo paciente" abre el modal
- Location: e2e\patients-list.spec.ts:116:3
...
Locator: locator('[role="dialog"], [aria-modal="true"]').or(locator('text=Nuevo paciente').nth(1))
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('[role="dialog"], [aria-modal="true"]').or(locator('text=Nuevo paciente').nth(1))
...
> 121 |     ).toBeVisible({ timeout: 5_000 });
  ...
  168 |       await expect(page).toHaveURL(/\/patients\/test-uuid-001\/history/, { timeout: 8_000 });
  169 |     } else {
  170 |       test.skip(true, 'No se encontró el campo nombre en el modal — revisar selectores');
  171 |     }
  172 |   });
  173 | 
  174 | });
