...
- Name: patients-list.spec.ts >> Lista de Pacientes >> UAT-PAT-12 | modal se puede cerrar con el botón cancelar
- Location: e2e\patients-list.spec.ts:124:3
...
Locator: locator('button').filter({ hasText: /[Cc]ancelar/ })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('button').filter({ hasText: /[Cc]ancelar/ })
