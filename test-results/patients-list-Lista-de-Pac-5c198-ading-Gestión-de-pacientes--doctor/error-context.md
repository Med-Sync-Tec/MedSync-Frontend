...
- Name: patients-list.spec.ts >> Lista de Pacientes >> UAT-PAT-01 | muestra el heading "Gestión de pacientes"
- Location: e2e\patients-list.spec.ts:30:3
...
Locator: locator('h1').filter({ hasText: 'Gestión de pacientes' })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('h1').filter({ hasText: 'Gestión de pacientes' })
