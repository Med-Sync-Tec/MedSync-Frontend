...
- Name: coo-dashboard.spec.ts >> Dashboard del COO >> UAT-COODASH-12 | filtro "Recientes" filtra artículos de las últimas 48h
- Location: e2e\coo-dashboard.spec.ts:92:3
...
Locator: locator('button').filter({ hasText: /Recientes/ }).first()
Expected: visible
Timeout: 8000ms
...
Call log:
  ...
  - waiting for locator('button').filter({ hasText: /Recientes/ }).first()
