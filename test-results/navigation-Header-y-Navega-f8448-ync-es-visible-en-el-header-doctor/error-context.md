...
- Name: navigation.spec.ts >> Header y Navegación — Doctor >> UAT-NAV-01 | logo "MedSync" es visible en el header
- Location: e2e\navigation.spec.ts:35:3
...
Locator: locator('header a').filter({ hasText: 'MedSync' })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('header a').filter({ hasText: 'MedSync' })
