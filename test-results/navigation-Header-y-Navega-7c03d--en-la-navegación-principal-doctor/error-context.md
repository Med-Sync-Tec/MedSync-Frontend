...
- Name: navigation.spec.ts >> Header y Navegación — Doctor >> UAT-NAV-04 | el link "Pacientes" está visible en la navegación principal
...
Locator: locator('nav a').filter({ hasText: 'Pacientes' }).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('nav a').filter({ hasText: 'Pacientes' }).first()
...
```ts
  1   | /**
  2   |  * UAT — Header y Navegación (transversal a todas las páginas)
  3   |  *
  4   |  * Cubre:
