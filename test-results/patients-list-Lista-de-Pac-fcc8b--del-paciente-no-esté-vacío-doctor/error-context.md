...
- Name: patients-list.spec.ts >> Lista de Pacientes >> UAT-PAT-13 | modal valida que el nombre del paciente no esté vacío
- Location: e2e\patients-list.spec.ts:132:3
...
Locator: locator('button').filter({ hasText: /[Gg]uardar|[Cc]rear/ })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('button').filter({ hasText: /[Gg]uardar|[Cc]rear/ })
