...
- Name: inventory.spec.ts >> Inventario de Medicamentos >> UAT-INV-02 | las columnas de la tabla están visibles
- Location: e2e\inventory.spec.ts:38:3
...
Locator: locator('th').filter({ hasText: 'Nombre' })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('th').filter({ hasText: 'Nombre' })
